// Metal native JNI for MetalRender (macOS only)
#import <Metal/Metal.h>
#import <QuartzCore/CAMetalLayer.h>
#import <Foundation/Foundation.h>
#import <jni.h>

// -------- Simple renderer holder --------
...
JNIEXPORT void JNICALL Java_pebblesboon_metalrender_MetalRuntime_nativeBeginFrame
  (JNIEnv* env, jclass cls, jlong handle) {
    ...
    rp.colorAttachments[0].clearColor = MTLClearColorMake(0.07, 0.12, 0.20, 1.0); // visible blue-ish clear
...
/**
 * Return an IOSurfaceID that backs the Metal texture so GL can bind it using CGLTexImageIOSurface2D.
 * For now we return 0 (not available). Implement by allocating a texture backed by IOSurface.
 */
JNIEXPORT jlong JNICALL Java_pebblesboon_metalrender_MetalRuntime_nativeGetIOSurfaceId
  (JNIEnv* env, jclass cls, jlong handle) {
    // TODO: Create IOSurface-backed MTLTexture and return IOSurfaceID
    return 0;
}
